...
input.forEach((line, index) => {
    ...
    line.split('').forEach((char, charIndex) => {
        ...
        //check if char is a number
        if (char === '*') {
            console.log('found gear', char, charIndex)
            let indexsOfNum = []
            if(index > 0) {
                input[index - 1].substring(charIndex - 1, charIndex + 2).split('').forEach((char, i) => {
                    if(!isNaN(char)){
                ...
                if(indexsOfNum.length > 0) {
                    indexsOfNum.forEach((ind) => {
                        let lineIndex = charIndex - 1 + ind;
                        gear.push(getWholeNum(input[index - 1], lineIndex))
                    })
                }
            }
            ...
            if(indexsOfNum.length > 0) {
                indexsOfNum.forEach((ind) => {
                    let lineIndex = charIndex - 1 + ind;
                    gear.push(getWholeNum(input[index], lineIndex))
                })
            }
            ...
            console.log('gear', gear, gear.length)
            if(gear.length == 2) {
                // console.log('gear', gear, gear.length)
                console.log('pushing gear')
                gears.push(gear[0] * gear[1])
            }
        }
    });
});
